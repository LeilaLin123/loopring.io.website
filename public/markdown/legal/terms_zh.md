...
对于因超出我们合理控制范围的任何行为或事件（“我们无法控制的事件”）而导致的任何未能履行或延迟履行我们在本协议下的义务的行为，我们不承担任何责任。特别要提醒您注意：**您接受（1）路印编写上传和路印应用程序集成的第三方编写上传至基础通证网络的代码和数据中的安全漏洞，（2）通证合约的升级和分叉，（3）基础通证网络的回滚和分叉均属于本条款所述的无法控制的事件的范畴。**
